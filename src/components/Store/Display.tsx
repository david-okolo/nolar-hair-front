              ...
              {category.products.map((item, index) => {
                  return (
                    <MenuCard 
                      ...
                      cardTitle={item.name}
                      extra={[
                      <Text style={{color: '#20274d', fontWeight: 300}}>{item.price}</Text>,
                      item.count !== 0 ? <Button type='text' style={{ color: '#20274d'}} onClick={() => {
                        props.addToCart(item)
                        push('/store/cart')
                      }}>Add To Cart</Button> : <Text delete style={{color: 'red'}}>Sold Out</Text>
                      ]}
                    ></MenuCard>
                  )
                })}
